...
class Base:
    ...
    Args:
        resource_manager (ResourceManager): The ResourceManager class which contains all tech and resource collection
        upgrade_level (BaseLevel): The current base level, starts as empty by default but can have the base start at
            any level if desired.
        base_number (int): The identifying base number. Mostly used for debugging purposes.
        pause_timer (int): Used to pause the building of a base. Typically done in the beginning of a simulation to
            mimic the amount of time it takes to clear a base
        print_mode (bool): Debugging purposes, will enable the print messages.
    ...
    def _default_build_slot_check(self, build_slot_name: str, tech_requirement: int, resource_requirement: int,
    ...
    def build_supply_pad(self, build_slot_name: str) -> BuildResult:
        """
    ...
    def build_temple(self, build_slot_name: str) -> BuildResult:
        """
    ...
    def upgrade_supply_pad(self, build_slot_name: str) -> BuildResult:
        """
    ...
    def _base_upgrade_helper(self, upgrade_cost: int, upgrade_timer: int) -> BuildResult:
        """
        Helper function for building and ugprdaing a base. Ensures the build checks are all consistent across all base
        upgrades and builds.

        :param upgrade_cost: int: The cost for the build/upgrade
        :param upgrade_timer: int: How long the build/upgrade should take
        :return: BuildResult: int: The result from the attempted build
    ...
    def is_supply_pad_built(self, build_slot_name: str) -> bool:
        """
        Helper function to check is supply pad has been built.

        :param build_slot_name: str: Name of build slot to check
        :return: True if supply pad has been built, false if not
        """
        # Check if there's an available build slot
        if build_slot_name in self.build_slots:
            if self.build_slots[build_slot_name].build_type == BuildSlotType.SUPPLY_PAD:
                if self.build_slots[build_slot_name].status == BuildSlotState.BUILT or self.build_slots[build_slot_name].status == BuildSlotState.UPGRADED:
                    return True
                else:
                    return False

    def is_supply_pad_upgraded(self, build_slot_name: str) -> bool:
        """
        Helper function to check if specified supply pad is upgraded.

        :param build_slot_name: str: Name of build slot to check
        :return: True if supply pad is upgraded, false if not
        """
        if build_slot_name in self.build_slots:
